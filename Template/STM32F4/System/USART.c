...
static void Usart_GPIO_Config()
{
    ...
    GPIO_PinAFConfig(SPECIFIE_USART_GPIO_PORT,SPECIFIE_USART_TX_PinSource,SPECIFIE_USART_TX_AF);    //USART GPIO 复用
    GPIO_PinAFConfig(SPECIFIE_USART_GPIO_PORT,SPECIFIE_USART_RX_PinSource,SPECIFIE_USART_RX_AF);    //USART GPIO 复用
    
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;       //设置 GPIO 50MHz
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;          //推挽输出
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;        //上下拉电阻不使能
    GPIO_InitStructure.GPIO_Pin = SPECIFIE_USART_TX_GPIO_PIN | SPECIFIE_USART_RX_GPIO_PIN;   //选择要控制的GPIO引脚
...
//    NVIC_PriorityGroupConfig(NVIC_PriorityGroup_4);                             //设置 NVIC 分组4
